      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">

        {
          projects.map((project, index)=>(    
            <motion.div key={project.id} initial={{ opacity: 0}} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, ease:"easeOut", delay: index * 0.1 }} className="group border border-gray-400 rounded overflow-hidden backdrop-blur-[2px] hover:backdrop-blur-[4px] rounded-lg transition">
              <div className="w-full relative h-60 bg-white overflow-hidden">
                ...
                <div className="w-full h-full absolute left-0 bottom-0 group-hover:bg-black/30 transition duration-500">
                  <div className="w-full h-full translate-y-[100%] group-hover:translate-y-0 transition duration-500">
                        <div className="w-full h-full flex justify-center items-center gap-2">
                          <button className='flex justify-center items-center gap-1 bg-black rounded px-4 py-1'>
                            <PiShare size={18}  />
                            <a href={project.link} className=''>Live Demo</a>
                          </button>
                          <button className='flex justify-center items-center gap-1 bg-black rounded px-4 py-1'>
                            <PiGithubLogoLight size={18} />
                            <a href={project.gitlink} className='inline-block'>Source</a>
                          </button>
                        </div>
                  </div>
                </div>
